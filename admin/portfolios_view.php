  ...
  jQuery('.huge-it-newuploader .button').click(function(e) {
	...
	jQuery('#portfolio_effects_list').on('change',function(){
		...
		jQuery('#portfolio_effects_list').change();
						...
						<a href="admin.php?page=portfolios_huge_it_portfolio&task=portfolio_deezer_widget&id=<?php echo $_GET['id']; ?>&TB_iframe=1" class="button button-primary add-video-slide thickbox"  id="slideup3s2" value="iframepop">
						<span class="wp-media-buttons-icon"></span><?php echo __( 'Add Deezer Widget', 'portfolio-gallery' );?>
						</a>
					</div>
					<ul id="images-list">
                                        <?php
...
/***</add>***/
function Html_portfolio_deezer_widget(){
	global $wpdb;
?>
	<style>
		html.wp-toolbar {
	...
	<script type="text/javascript">
		...
		jQuery('.huge-it-insert-deezer_wifget-button').click(function(){
			...
			//	jQuery("#save-buttom").click();
				...
				/*jQuery('#huge_it_add_deezer_widget_input').change(function(){
					
					if (jQuery(this).val().indexOf("youtube") >= 0){
						jQuery('#add-deezer_widget-popup-options > div').removeClass('active');
						jQuery('#add-deezer_widget-popup-options  .youtube').addClass('active');  // Changing
					}else if (jQuery(this).val().indexOf("vimeo") >= 0){
						jQuery('#add-video-popup-options > div').removeClass('active');
						jQuery('#add-video-popup-options  .vimeo').addClass('active');
					}else {
	...
	<div id="huge_it_slider_add_deezer_widget">
		<div id="huge_it_slider_add_deezer_widget_wrap">
			<h2><?php echo __( 'Add Deezer Widget', 'portfolio-gallery' );?></h2>
			<div class="control-panel">
			<?php if (!isset($_GET['thumb_parent'])) { ?>
		...
		jQuery('#portfolio_effects_list').change();
			...
			<?php } else { $thumb_parent = $_GET["thumb_parent"] //get project image's id and sent to form by get ,who addes thumb_video by $_get thumb_parent ?>
				<form method="post" action="admin.php?page=portfolios_huge_it_portfolio&task=portfolio_deezer_widget&id=<?php echo $_GET['id']; ?>&thumb_parent=<?php echo $thumb_parent ; ?>&closepop=1" >
					...
					<button class='save-slider-options button-primary huge-it-insert-video-button' id='huge-it-insert-deezer_widget-button'><?php echo __( 'Insert Deezer Widget', 'portfolio-gallery' );?></button>
				</form>
			<?php } ?>
			</div>
		</div>	
